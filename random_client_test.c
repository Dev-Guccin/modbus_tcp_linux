...
*/
    ...
    /* TCP */
    ctx = modbus_new_tcp("127.0.0.1", 1502);  //Modbus TCP/IPv4 서버와 통신하기 위해 modbus_t 구조를 할당하고 초기화
    modbus_set_debug(ctx, TRUE);    //인수 flag를 사용하여 modbus_t 컨텍스트의 디버그플래그를 설정
    ...
    //머하는 건지 잘 모르겠다. 걍 어드레스만큼 새로운 공간을 힙에 할당한다.
    nb = ADDRESS_END - ADDRESS_START; //99임
